...
void Mesh_texture()
{
  ...
  pcl::io::loadPCDFile ("test_32.pcd", *cloud);
  
  Eigen::Matrix3d cmtx;
  Eigen::Matrix4d tmtx;
  
  read_parm("calibration_params.yml",cmtx ,tmtx);
  ...
  auto triangle = Generate_mesh(cloud);

  for(auto &tri: triangle.polygons)
  {
...
{
  cv::Mat img = cv::imread("test_20.png");
  cv::flip(img, img, 0); 
